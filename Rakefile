...
$: << File.join(ROOT_PATH, 'fonsovim', 'ruby')
...
task :link_vim_conf_files do
  %w[ vimrc gvimrc ].each do |file|
    ...
    unless File.exist?(dest)
      require 'rbconfig'
      source = expand("../fonsovim/vim/#{file}", __FILE__)
...
task :folders do
  Fonsovim::VIM.folders.each do |folder|
    mkdir_p folder
  end
end
...
task :update do
  ...
  puts "Cleaning the fonsovim folder"
  `git clean -xdf -- fonsovim #{redirect_out_err}`
  `git ls-files --exclude-standard --others -- fonsovim`.split("\n").each do |untracked|
...
desc "Install or Update Fonsovim."
task :default do
  sh "rake update"
  sh "rake install"
end
